...
//2s	SENT TCP DATA
#define TCPSent_CREATTRM_INTERVAL (250*4)*1 
//500ms	SENT UDP DATA
#define UDPSent_CREATTRM_INTERVAL (250*4)*2
...
void udp_client_callback_app(void)
{
  ...
  extern uint32_t smtp_Tcp_count[20];
 ...
 if (err == ERR_OK && p != NULL)
  {
    ...
    tcp_recved(tpcb, p->tot_len);
		 data = p->payload;	 
					...
					if (strncmp(data, "smtp", 4) == 0)
					 {
						   char *ptr;
						   uint16_t len;

		                 ptr=(char *)smtp_Tcp_count;
						 len=80;
...
if (Time - TCPCreatTrm >= TCP_CREATTRM_INTERVAL)
 ...
 u8 Tcpbf[2048]={0xaa,0x55};
char num=0,num1=0;
uint32_t TCPSentCreatTrm=0;
extern char mailsentbox[18];
 void SendDataToSever(uint32_t Time)
 {			err_t erro;
	
  
 		 if((Tcplink==0)&&(cansent)&&((Time - TCPSentCreatTrm >= TCPSent_CREATTRM_INTERVAL)))
		 {
		        TCPSentCreatTrm =  Time;
		   //连接成功啦可以发送数据

					 memcpy(Tcpbf,mailsentbox,18);
				   erro= tcp_write(wpcb, Tcpbf, 2048, 1);
